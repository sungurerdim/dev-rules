...
Every production service needs structured logging, metrics, + health checks.
...
**Correlation IDs:** Generate unique ID per request. Propagate through all downstream calls (HTTP headers, queue messages, background jobs). Include in every log entry.
...
**Health check endpoints:** `/health` returns 200 when service operational. `/ready` returns 200 when service can accept traffic (DB connected, cache warm, dependencies reachable).
...
Every cache interaction must define: TTL, invalidation strategy, + stale-data tolerance.
...
**Cache stampede prevention:** Popular cache key expires → many requests hit DB simultaneously. Mitigate with: stale-while-revalidate, mutex/lock on cache miss, or probabilistic early expiration.

---

## Configuration Validation

**Startup validation:** Check all required config values before accepting traffic. Missing/invalid config → fail fast with clear error message listing every missing variable.

**Environment separation:**
- `.env.example` in repo with placeholder values + comments — never real secrets
...
**Expand-contract pattern:** Changing column/table → expand first (add new), migrate data, update code, then contract (remove old). Allows previous app version to function during deployment.

**Safe migration checklist:**

| Step | Verification |
|------|-------------|
| 1. Write migration | Both `up` + `down` operations defined |
...
| Check | Verification |
|-------|-------------|
| Rollback plan | Previous version restorable within 5 minutes |
| Health checks | `/health` + `/ready` endpoints respond correctly |
| Graceful shutdown | App handles SIGTERM, drains connections, completes in-flight requests |
...
**Agent-authored changes:** A PR opened by an autonomous agent gets human review before merge when it touches CI/CD (`.github/workflows/`, pipelines), infrastructure-as-code, auth, or secrets — and never auto-merges to a protected branch. Agent dependency-bumps included. Source: [When AI Agents Touch CI/CD Configurations — 8,031 agentic PRs (arXiv:2601.17413, 2026)](https://arxiv.org/abs/2601.17413).
...
**Post-deployment:** Monitor for 15 minutes after full rollout. Compare error rates, latency, + business metrics against pre-deployment baseline.
