...
**Tier 1** — 3–7 files, clear scope, no destructive ops: state `→ [goal] | [files] | done: [criteria]` and proceed — no confirmation needed unless the user intervenes.
...
Split work so each unit stays below the reliable horizon: ≤ ~5 files and ≤ ~25 tool calls. Larger → sequence into independently verifiable units in `tasks.md`.
...
Before reporting done: machine check green (test/type-check/lint/build for touched scope)? | `Done:` criteria met? | `tasks.md` complete? | re-read modified files | `git diff` clean | no residue: scratch files, debug scripts, one-off helpers, completed `tasks.md` deleted (or per user instruction) | output syntactically complete — no truncation, TODOs, or stubs | state: what changed + how to verify.
...
**Context hygiene (W18):** Front-load task constraints; summarize intermediate results instead of accumulating raw output — don't assume early context stays salient. Re-confirm values read early before acting on them late. Re-grounding cadence: Artifact-First Recovery. Stuck-loop threshold: Process Framework › repeated failure (3×).

**LSP-First Navigation [GATE]** — typed codebases (Go, Python, Dart, TypeScript): use LSP before text search. Grep fallback: LSP unavailable or untyped code (Markdown, Bash, JSON).
