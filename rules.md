...
**Scope Boundary:** Touch only lines the task requires. Unrelated issues → record, don't fix (record where deferred work survives — see Outcome Report). Never reformat untouched code, reorder unrelated imports, or change whitespace in unmodified lines.
